...
function SummaryCard({ icon, label, value, sub, accent, addTo, addTab }: { icon: ReactNode; label: string; value: string; sub: string; accent: string; addTo?: string; addTab?: string | undefined }) {
  ...
  return (
    <CardWrapper
      ...
      className="rounded-2xl bg-[var(--bg-card)] pt-8 pb-3 px-3 pr-8 shadow-sm ring-1 ring-[var(--border-color)] overflow-hidden relative block min-h-[88px]"
      style={{ borderTop: `3px solid ${accent}` }}
    >
      <span
        className="absolute right-4 top-4 flex h-6 w-6 items-center justify-center rounded-full text-[11px] font-bold leading-none text-white shadow-sm"
