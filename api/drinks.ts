...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'POST') {
    ...
    if (!validateLengths(res, [['type', type, MAX_LENGTHS.shortText], ['notes', notes, MAX_LENGTHS.notes]])) return;
  ...
  if (req.method === 'PUT') {
    ...
    // Fetch existing for audit and verify the entry belongs to an accessible child
    const existing = await sql`SELECT date, time, type, amount_ml, notes, child_id FROM drink_entries WHERE id = ${id}`;
  ...
  if (req.method === 'DELETE') {
    ...
    const toDelete = await sql`SELECT child_id FROM drink_entries WHERE id = ${id}`;
