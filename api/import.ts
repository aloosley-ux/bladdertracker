...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (drinks) {
    for (const [i, entry] of (drinks as EntryPayload[]).entries()) {
      ...
      await sql`
        ...
        VALUES (${generateId()}, ${childId}, ${entry.date}, ${entry.time}, ${entry.type || entry.drinkType || 'cup'}, ${entry.amountMl}, ${entry.notes || ''}, ${session.userId})
      `;
      summary.drinks++;
    }
  }
  ...
  if (urineEntries) {
    for (const [i, entry] of (urineEntries as EntryPayload[]).entries()) {
      ...
      await sql`
        ...
        VALUES (${generateId()}, ${childId}, ${entry.date}, ${entry.time}, ${Boolean(entry.wet)}, ${Boolean(entry.pass)}, ${entry.notes || ''}, ${session.userId})
      `;
  ...
  if (bowelEntries) {
    for (const [i, entry] of (bowelEntries as EntryPayload[]).entries()) {
      ...
      await sql`
        ...
        VALUES (${generateId()}, ${childId}, ${entry.date}, ${entry.time}, ${entry.location || 'toilet'}, ${entry.amount || 'M'}, ${entry.bristolType || 4}, ${Boolean(entry.laxativesGiven)}, ${entry.notes || ''}, ${session.userId})
      `;
  ...
  await sql`
    ...
    VALUES (${generateId()}, ${session.userId}, 'Imported diary data', ${childId}, ${`Imported ${total} records.`})
  `;
